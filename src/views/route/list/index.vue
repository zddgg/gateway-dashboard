        ...
        <template #operations="{ record }">
          <a-space>
            ...
            <a-button
              v-permission="['admin']"
              size="small"
              @click="deleteRoute(record.routeId)"
            >
              删除
            </a-button>
          </a-space>
...
<script lang="ts" setup>
  ...
  const renderData = ref<RouteRecord[]>([]);
  const formModel = ref<RouteRecord>({} as RouteRecord);
  ...
  const fetchData = async (
    params: RouteParams = { current: 1, pageSize: 20 }
  ) => {
    setLoading(true);
    try {
  ...
  fetchData();
  const reset = () => {
    formModel.value = {} as RouteRecord;
  };
  ...
  const deleteRoute = async (routeId: string) => {
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        const { msg } = await deleteByRouteId(params);
        Message.info(msg);
        await fetchData();
      },
    });
